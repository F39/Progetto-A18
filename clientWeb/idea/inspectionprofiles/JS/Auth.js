...
function get_signin_data(){
    ...
    if(password == c_password ){
        ...
        var xhr = RESTrequest('POST', "/user/create", data)
        xhr.onreadystatechange = function () {
            if (xhr.readyState === 4 && xhr.status === 200) {
                console.log(xhr.responseText);
                //console.log(json.email + ", " + json.password);
                alert("Registrazione effettuata con successo"); //non sapevo come dirlo in inglese
...
function check_token(){
    ...
    /*Define endpoint to token verification*/
    ...
    xhr.onreadystatechange = function () {
        if (xhr.readyState === 4 && xhr.status === 200) {
            ...
            alert("YOU SHALL NOT PASS!"); //giuro che lo cambierò
        }
    };  
